...
| 30. API-004 实现商品、价格历史和通知查询         | completed   | 2026-08-03 完成：查询端点 + 4 测试，提交 e95cb3e。                                                                                                                                                                                                |
| 31. 后续工单                                     | in_progress | MVP 后端 P0 核心链路全部完成（采集契约→清洗→基线→低价判定→匹配→通知→API）。剩余：**P1 工单**（PRI-004 基线缓存、NTF-003/004 渠道投递（EXT-002）、WEB-* 前端、DB 索引优化）、**FND-001 剩余**（ADR-001/ADR-002）、**COL-002**（被 EXT-001 阻塞）。 |
