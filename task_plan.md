...
| 25. 后续 P0 工单                                 | in_progress | 依赖已解锁：**PRI-003**（商品事件匹配监控任务，依赖 API-003⛔/PROC-005✅/PRI-002✅——API-003 阻塞）、**PRI-004**（P1 基线缓存）、**NTF-001**（通知事件/模板，依赖 PRI-003/API-003 均⛔）。**FND-001 剩余**（ADR-001/ADR-002）可随时补；**API-001**（NestJS 初始化）依赖 FND-002/003/006✅ + DB-005✅ 已全部满足→可做。 |
