...
| 13. 后续 P0 工单                             | in_progress | 可执行候选：**FND-004**（Docker Compose 配置，本机 docker 缺失验收受限，但可交付配置解锁 DB-003/API-001 依赖）、**FND-001 剩余**（ADR-001/ADR-002 补写）。DB-003/API-001 依赖链仍被 FND-004 阻塞；COL-002 被 EXT-001 阻塞。 |
