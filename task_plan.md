...
| 26. 后续 P0 工单                                 | in_progress | 依赖已解锁：**API-002**（用户注册/登录/会话，依赖 API-001/DB-001✅）→ 认证链路；**API-003**（监控 CRUD，依赖 API-002/DB-002）→ 解锁 **PRI-003**（事件匹配监控，依赖 API-003/PROC-005/PRI-002）。FND-001 剩余（ADR-001/ADR-002）可随时补；COL-002/COL-004 被 EXT-001 阻塞。 |
