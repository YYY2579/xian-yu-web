...
| 14. 后续 P0 工单                             | in_progress | 依赖已解锁：**DB-003**（商品与价格历史模型，依赖 FND-004✅，可用嵌入式 PG 完整验证）→ 解锁 PROC/PRI 链路；**COL-003**（调度锁/配额，需 Redis，本机无 redis 验证受限）。FND-001 剩余（ADR-001/ADR-002）可随时补。COL-002 仍被 EXT-001 阻塞。 |
