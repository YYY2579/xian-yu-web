...
| 28. 后续 P0 工单                                 | in_progress | 依赖已解锁：**PRI-003**（商品事件匹配监控任务，依赖 API-003/PROC-005/PRI-002✅）→ 打通分析→通知；**NTF-001**（通知事件/模板，依赖 PRI-003/API-003✅）；**API-004**（商品/价格历史查询，依赖 API-001/DB-003/DB-004✅）。FND-001 剩余（ADR-001/ADR-002）可随时补。 |
