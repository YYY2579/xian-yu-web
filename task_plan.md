...
| 16. 后续 P0 工单                             | in_progress | 依赖已解锁：**PROC-001**（商品标准化解析器，依赖 COL-001/DB-003✅，纯单元测试可完整验证）→ 解锁 PROC 链路；**DB-005**（P1：索引/迁移/种子，依赖 DB-001~004✅）。FND-001 剩余（ADR-001/ADR-002）可随时补。COL-003 需 Redis（本机验证受限）；COL-002 被 EXT-001 阻塞。 |
